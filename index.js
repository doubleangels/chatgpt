...
for (const file of eventFiles) {
  ...
  const eventHandler = (...args) => {
    ...
    event.execute(...args, client).catch(err => {
      logger.error("Error executing event:", { event: event.name, error: err });
    });
  };
...
client.on('interactionCreate', async interaction => {
  if (interaction.isChatInputCommand()) {
    ...
    try {
      ...
      logger.error(`Error executing chat input command:`, { command: interaction.commandName, error });

      const responseMessage = 'There was an error executing that command!';
      try {
  ...
  if (interaction.isContextMenuCommand()) {
    logger.debug("Executing context menu command:", { command: interaction.commandName, user: interaction.user.tag });
    ...
    try {
      ...
      logger.error(`Error executing context menu command:`, { command: interaction.commandName, error });
